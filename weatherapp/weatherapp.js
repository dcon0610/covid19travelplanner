    ...
    //Icons are searched from the relevant URL, and the icon code found in the API. 
      $.ajax({
        ...
        //15 is 3 pm in 24 hour time
         ...
         $("#cards").empty()
        $('#forecast').html('<h2 class= "margin_outside  is-medium">'+"5-Day Forecast:"+'</h2>')

        for (i=0; i<5; i++){
          var $div = $("<div>", {id: i, class: "card column",});
          var $insidediv = $("<div>",{id: "insideDiv", class: "card-content has-background-info has-text-info-light"})
          $insidediv.html('<h5 class="card-header-title">'+ forecastDate[i]+ '</h5>')
        
          var image='<img src="http://openweathermap.org/img/wn/'+forecastIcon[i] +'@2x.png" width="60" height="60" ></img>'
          $insidediv.append('<p class="card-content">'+ image + '<p>')
          $insidediv.append('<p class="card-content">'+ "Temp: " + forecastTemp[i] +" °C" + '<p>')
          $insidediv.append('<p class="card-content">'+ "Humidity: " + forecastHumidity[i] +" %" + '<p>')
